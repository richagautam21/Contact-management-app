...
const ContactItems: React.FC<ContactItemsProps> = ({ contactDetail }) => {
  ...
  return (
    <li className="bg-white m-5 w-[180px] h-[250px] border text-[#36454F] font-grey p-3 flex flex-col items-center justify-center font-bold rounded-xl shadow-lg border-[#f4544c]">
      ...
      <div className="">
        <button onClick={clickedToEdit} className="block bg-[#FF0000] w-20 m-3 rounded-xl h-15 text-white" type="button">
          Edit
        </button>
        <button onClick={clickToDeleteItem} className="block bg-[#FF0000] w-20 m-3 rounded-xl h-15 text-white" type="button">Delete</button>
      </div>
    </li>
  );
};
